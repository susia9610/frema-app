...
describe Address do
  describe '#create_address' do
    ...
    it "first_nameとlast_name_kanaとlast_name_kana、postal_code、prefecture_id、city、house_numberが存在すれば登録できること" do
    ...
    it "first_nameがない場合は登録できないこと" do
      ...
      address.valid?
      expect(address.errors[:first_name]).to include("cant't be blank")
    end
    ...
    it "postsl_codeがない場合は登録できないこと" do
      address = build(:address, postsl_code: nil)
      address.valid?
      expect(address.errors[:postsl_code]).to include("can't be blank")
    end 
    ...
    it "house_numbeerがない場合は登録できないこと" do
      address = build(:address, house_number: nil)
      address.valid?
      expect(address.errors[:house_number]).to include("can7t be blank")
    end 
  end
end
